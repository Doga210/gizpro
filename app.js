...
const TRANSLATIONS = {
    ar: {
        ...
        settings: 'إعدادات',
        logout: 'تسجيل الخروج',
        tonConnect: 'ربط محفظة TON',
        gizAddress: 'عنوان GIZ الخاص بك',
        shareAddress: 'شارك هذا العنوان لاستقبال GIZ',
        buy: 'شراء',
        price: 'السعر',
        daily: 'يومياً',
        perClick: 'نقرة',
        ...
        cancel: 'إلغاء',
        copied: 'تم النسخ!',
        depositTon: 'أرسل TON إلى هذا العنوان',
        levelExpired: 'انتهت مدة مستواك!',
        insufficientBalance: 'رصيد غير كافٍ',
        phoneVerify: 'تحقق من رقم الهاتف',
        enterCode: 'أدخل كود التحقق',
        refCode: 'كود الإحالة',
        yourRefCode: 'كود الإحالة الخاص بك',
        shareRef: 'شارك الكود',
        profit: 'ربح',
    ...
    en: {
        ...
        gizAddress: 'Your GIZ Address',
        shareAddress: 'Share this address to receive GIZ',
        buy: 'Buy',
        price: 'Price',
        daily: 'Daily',
        perClick: 'Per Click',
        ...
        cancel: 'Cancel',
        copied: 'Copied!',
        depositTon: 'Send TON to this address',
        levelExpired: 'Your level has expired!',
...
function applyLanguage() {
    ...
    // تحديث العنوان
    document.title = currentLang === 'ar' ? 'Gizpro - محفظة GIZ' : 'Gizpro - GIZ Wallet';
...
function updateDynamicTexts() {
    ...
    // تحديث أزرار Action
    const btnSend = document.querySelector('#btn-send span');
    if (btnSend) btnSend.textContent = t.send;
    
    const btnReceive = document.querySelector('#btn-receive span');
    if (btnReceive) btnReceive.textContent = t.receive;
    
    const btnSwap = document.querySelector('#btn-swap span');
    if (btnSwap) btnSwap.textContent = t.swap;
    
    const btnHistory = document.querySelector('#btn-history span');
    ...
    // تحديث TON Status
    const tonStatus = document.querySelector('.ton-status-desc');
    if (tonStatus) tonStatus.textContent = t.tonConnect;
    
    // تحديث Address
    const addressNote = document.querySelector('.address-note');
    if (addressNote) addressNote.textContent = t.shareAddress;
}
...
const TRANSLATIONS = {
  ar: {
    ...
    recipientAddress: 'عنوان GIZ المستلم',
    amount: 'المبلغ',
    note: 'ملاحظة',
    ...
    connected: 'متصل',
    connectTON: 'ربط محفظة TON',
    disconnectTON: 'فصل محفظة TON',
    ...
    days: 'أيام',
    expired: 'انتهى',
    subWallets: 'محافظي GIZ الفرعية',
    createWallet: 'إنشاء محفظة فرعية',
  ...
  en: {
    ...
    recipientAddress: 'Recipient GIZ Address',
    amount: 'Amount',
    note: 'Note',
    ...
    disconnectTON: 'Disconnect TON Wallet',
    ...
    days: 'days',
    expired: 'Expired',
    subWallets: 'My GIZ Sub-Wallets',
    createWallet: 'Create Sub-Wallet',
    walletName: 'Wallet Name',
    chooseColor: 'Choose Color'
  }
};

let currentLang = localStorage.getItem('gizpro_lang') || 'ar';
...
function toggleLanguage() {
  currentLang = currentLang === 'ar' ? 'en' : 'ar';
  localStorage.setItem('gizpro_lang', currentLang);
  document.documentElement.dir = currentLang === 'ar' ? 'rtl' : 'ltr';
  document.documentElement.lang = currentLang;
  applyTranslations();
}
